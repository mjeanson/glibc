...
__libc_setup_tls (void)
{
  ...
  /* Get the rseq auxiliary vectors, 0 is returned when not implemented
     and we then default to the rseq ABI minimums. When rseq is disabled just
     use the minimums.  */
  ...
  if (do_rseq)
    {
      rseq_size = MAX (GLRO(dl_tls_rseq_feature_size), TLS_DL_RSEQ_MIN_SIZE);
      rseq_align = MAX (GLRO(dl_tls_rseq_align), TLS_DL_RSEQ_MIN_ALIGN);
    }
...
#elif TLS_DTV_AT_TP
  /* After the thread pointer, add the non-aligned tls block size and then
     align the rseq area block on top.  */
  tls_blocks_size = roundup (memsz + rseq_size, rseq_align);
 ...
 /* Record the rseq_area offset. The offset is positive with TLS_DTV_AT_TP
  ...
  tlsblock = _dl_early_allocate (tcb_offset + tls_blocks_size + max_align
				 + TLS_PRE_TCB_SIZE
